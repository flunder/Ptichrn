...
import { usePlaylistID } from './usePlaylistID'
...
function useRecordsFromJuno(props) {

    const [results, setResults] = useState([]);
    const playlistId = usePlaylistID();

    // 1. First get the playlistID from juno
    // 2. then use this id to load the playlist
    // 3. parse and reformat that xml feed into sweets

    useEffect(() => {
        if (playlistId) getPlaylistFromJuno();
    }, [playlistId]);
    ...
    const getPlaylistFromJuno = async () => {
        fetch(`https://www.juno.co.uk/playlists/builder/${playlistId}.xspf)`)
